...
def render_html_content(
    ...
    Args:
        report_data: Dictionary containing stats, failed_ids, new_titles, etc.
        ...
        update_info: Optional version update information

    Returns:
        Complete HTML string for the report
    """
    # Determine report type text
    if is_daily_summary:
        if mode == "current":
            report_type = "当前榜单"
        elif mode == "incremental":
    ...
    # Calculate hot news count
    hot_news_count = sum(len(stat["titles"]) for stat in report_data["stats"])

    # Get current time
    now = get_beijing_time()
    generated_time = now.strftime("%m-%d %H:%M")
    ...
    <body>
        <div class="container">
            {render_header(report_type, total_titles, hot_news_count, generated_time)}

            <div class="content">
                {render_error_section(report_data["failed_ids"])}"""

    # Render word groups
    if report_data["stats"]:
        total_count = len(report_data["stats"])
        for i, stat in enumerate(report_data["stats"], 1):
            html += render_word_group(stat, i, total_count)

    # Render new titles section
    html += render_new_section(
    ...
    html += render_footer(update_info)
